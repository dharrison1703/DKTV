...
def AddTVSHOWDir(name, url, mode, iconimage, description="", isFolder=True, background=None):
	...
	liz = xbmcgui.ListItem(name, iconImage=iconimage, thumbnailImage='http://devil666wizard.x10host.com/BackgroundArt/Icon.png')
        ...
        for link in Links:
            if '.gif' in link:
                ...
                if '.mkv' in link or '.mp3' in link or '.mp4' in link or '.avi' in link or '.flv' in link or '.divx' in link:
                    AddTVSHOWDir(name, url+link, 404,'http://devil666wizard.x10host.com/BackgroundArt/KidsIcon.png', 'The author neither hosts or stores any content, the author has no affiliation with content hosts', isFolder=False)
					
                else:
					AddTVSHOWDir(name, url+link, 22, '', '', isFolder=True)
